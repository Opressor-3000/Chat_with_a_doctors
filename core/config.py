...
class DBSettings(BaseModel):
    url: str = (
        "postgresql+asyncpg://yusif:BTKChat@localhost:5433/btk_db?async_fallback=True"
    )
    url1: str = (
        "postgresql+asyncpg://%(DB_USER)s:%(DB_PASS)s@%(DB_HOST)s:5433/%(DB_NAME)s?async_fallback=True"
    )
